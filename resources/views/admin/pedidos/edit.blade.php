...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pedido.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pedidos.update", [$pedido->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $cliente)
                        <option value="{{ $id }}" {{ ($pedido->cliente ? $pedido->cliente->id : old('cliente_id')) == $id ? 'selected' : '' }}>{{ $cliente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nombre_produto') ? 'is-invalid' : '' }}" name="nombre_produto_id" id="nombre_produto_id" required>
                    @foreach($nombre_produtos as $id => $nombre_produto)
                        <option value="{{ $id }}" {{ ($pedido->nombre_produto ? $pedido->nombre_produto->id : old('nombre_produto_id')) == $id ? 'selected' : '' }}>{{ $nombre_produto }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tamanio') ? 'is-invalid' : '' }}" name="tamanio_id" id="tamanio_id" required>
                    @foreach($tamanios as $id => $tamanio)
                        <option value="{{ $id }}" {{ ($pedido->tamanio ? $pedido->tamanio->id : old('tamanio_id')) == $id ? 'selected' : '' }}>{{ $tamanio }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad') ? 'is-invalid' : '' }}" type="number" name="cantidad" id="cantidad" value="{{ old('cantidad', $pedido->cantidad) }}" step="1" required>
